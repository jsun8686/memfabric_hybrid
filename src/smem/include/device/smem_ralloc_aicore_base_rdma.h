...
/**
 * @brief Debug helper: dump one connection context into a 128B device buffer for bring-up.
 */
                                                            ...
                                                            __gm__ uint8_t *out)
{
    ...
    *(__gm__ uint64_t *)(out + 16) = smem_ralloc_get_qp_info_address(entityId) == NULL ? 0 : 1;
    __gm__ SmemRallocRdmaInfo *rdmaInfo = (__gm__ SmemRallocRdmaInfo *)qpInfoVa;
    *(__gm__ uint64_t *)(out + 24) = rdmaInfo->qpNum;
    __gm__ SmemRallocWQCtx *qpCtxEntry =
        (__gm__ SmemRallocWQCtx *)(rdmaInfo->sqPtr + (destRankId * rdmaInfo->qpNum + qpIdx) *
                                                        sizeof(SmemRallocWQCtx));
    *(__gm__ uint64_t *)(out + 32) = qpCtxEntry->bufAddr;
    *(__gm__ uint64_t *)(out + 40) = qpCtxEntry->wqeSize;
    *(__gm__ uint64_t *)(out + 48) = qpCtxEntry->depth;
    smem_ralloc_cache_write_through((__gm__ uint8_t *)qpCtxEntry->headAddr, 8);
    *(__gm__ uint64_t *)(out + 56) = *(__gm__ uint32_t *)(qpCtxEntry->headAddr);
    smem_ralloc_cache_write_through((__gm__ uint8_t *)qpCtxEntry->tailAddr, 8);
    *(__gm__ uint64_t *)(out + 64) = *(__gm__ uint32_t *)(qpCtxEntry->tailAddr);
    *(__gm__ uint64_t *)(out + 72) = (uint64_t)qpCtxEntry->dbMode;
    *(__gm__ uint64_t *)(out + 80) = qpCtxEntry->dbAddr;
    __gm__ SmemRallocMemInfo *remoteMemInfo = (__gm__ SmemRallocMemInfo *)(rdmaInfo->memPtr +
                                                                           sizeof(SmemRallocMemInfo) * destRankId);
    *(__gm__ uint64_t *)(out + 88) = remoteMemInfo->rkey;
    *(__gm__ uint64_t *)(out + 96) = remoteMemInfo->addr;
    __gm__ SmemRallocMemInfo *localMemInfo = (__gm__ SmemRallocMemInfo *)(
        rdmaInfo->memPtr + sizeof(SmemRallocMemInfo) * smem_ralloc_get_global_rank(entityId));
    *(__gm__ uint64_t *)(out + 104) = localMemInfo->lkey;
    *(__gm__ uint64_t *)(out + 112) = localMemInfo->addr;
    *(__gm__ uint64_t *)(out + 120) = localMemInfo->size;
}
