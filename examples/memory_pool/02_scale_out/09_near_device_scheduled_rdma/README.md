...
- 本用例的池以 `DEVICE_RDMA | DEVICE_SCHEDULE` 创建：trans 层改建 AI 核 QP 并把
  ...
  自 quiet**，全程零 host 交互；
- host 侧 `submit_device_write` 只做一次内核入队 → 整个写作业（iters 次单边 WRITE +
  quiet）可被 NPUGraph 捕获，**捕获一次、重放 K 次**，重放期间 host 完全不参与。
...
│ （executor 自动以          │◄──────────│ NPUGraph: submit_device_write       │
│  AI_CORE_INITIATE 加入池） │ RDMA WRITE │   ├─ iters 次 AICore 单边写 + quiet │
│                            │  (AICore   │ verify: submit_device_read 回读比对 │
...
- `handle.submit_device_write` / `handle.submit_device_read` / `handle.get_entity_id`
  （安装期编译的随包内核，`dlopen` 惰性加载）
- NPUGraph 捕获与重放（`torch.npu.NPUGraph`），warmup 必须在捕获外完成（首次 submit 会
  `dlopen` + 加载内核，捕获中非法）
...
| `--size` | 1M | 每次单边 RDMA WRITE 的字节数（K/M/G 后缀） |
| `--iters` | 4 | 一张图内的写次数（图内 iters 次 WRITE + 1 次 quiet） |
| `--replays` | 3 | 捕获后重放次数（计时段） |
| `--block-size` | 32M | 两侧各提交的 HBM slot 字节数（须 ≥ 2×size：slot 布局 `[src \| verify]`） |
...
- 安装期 `install ralloc device rdma lib success`（bisheng 在位；失败会 WARNING 跳过，
  客户端 `submit_device_write` 将报 library not available）
...
- 客户端关键行依次出现：
  ...
  3. `graph captured: <iters> x <size> byte RDMA WRITE + quiet, no host interaction inside`
...
## 判读
- replay 吞吐 = `replays × iters × size / 计时`（计时含末次 synchronize）；单边 WRITE
  串行提交 + 卡内 quiet，吞吐参考受消息粒度与 QP 深度约束，本用例验证的是**可捕获性
  与正确性**，不是极限带宽
...
| 症状 | 处置 |
|---|---|
| submit 返回非 0 且日志报 library not available | 安装期 bisheng 缺失或编译失败：确认 `bisheng` 在 PATH、重跑 install.sh，检查 `lib64/libmf_smem_ralloc_device_rdma.so` 是否存在 |
