...
┌────────────────────────────┐          ┌─────────────────────────────────────┐
│ contributor 贡献 DRAM slot │          │ torch 张量 src1/src2/dst（HBM，    │
│  ├─ slice 1: 64M（独立 MR）│◄─────────│ register，双 pattern 源/读回落点）   │
│  └─ slice 2: 64M（独立 MR）│ RDMA     │ 本地 slice 1/2（64M，W2 下载落点）  │
│  （每 rank 2 条 memoryMap）│  WRITE/  │ NPUGraph: device_copy ×2             │
│                            │  READ    │   ├─ 槽 0/1 区间匹配取 rkey/lkey     │
│                            │          │ 捕获 1 次（双写）→ replay K 次       │
└────────────────────────────┘          └─────────────────────────────────────┘
```
...
# 2) NEAR 节点跑客户端：建池 → 扩容 2+2 → 注册张量 → warmup → 捕获 → 重放 → 双重校验
...
- **两端版本一致**：MR 表多槽布局（`[rankCount × 8]`）为 host 库与设备头的硬契约
  ...
  同包发布），不得混版本运行
- 每 rank slice 数 > 8 时第 9 个起设备侧不可见（host WARN + 截断），需要更多槽时
  扩 `MR_SLOTS_PER_RANK` 并整包同步三处定义

## 验收标准
- 客户端关键行依次出现：
  1. `device-scheduled pool created (DEVICE_RDMA | DEVICE_SCHEDULE)`
  2. `pool grown to 2+2 slices: FAR rank <R> slots [0x..., 0x...)`（两段 gva 不重叠）
  3. `patterns registered: <size> bytes each (seeds ...)`
...
| `copy_data` 报 `no host copy operator` | 预期行为：device-scheduled 池不支持 host copy，改用 device_copy（本用例即此） |
| verify 报 pattern 交叉 | MR 槽数据错位（应回看 FillQpInfo 日志）；全零/脏数据 = 链路或 CQE 异常 |
