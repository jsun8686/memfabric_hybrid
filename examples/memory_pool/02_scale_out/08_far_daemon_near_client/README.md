...
# 3) 停止守护（或前端 Ctrl+C）
kill -TERM <daemon_pid>        # pgrep -f memfabric_daemon 找 pid
```
...
| `--io-sizes` | 1M,8M | 拷贝粒度列表（K/M/G 后缀） |
| `--batch-size` | 64M | 每粒度单向拷贝量，块数 = batch-size / 粒度（K/M/G 后缀）；不足一个粒度至少跑 1 块 |
| `--real-pool-size` | 64M | 从 FAR 池实际申请的远端块大小（K/M/G 后缀；须 ≥ 最大粒度且 ≤ max-pool-size） |
| `--max-pool-size` | 4G | 池窗口，即 `ralloc.create` 的 `max_dram_size`（上报 FAR 放置 master） |
...
| 症状 | 处置 |
|---|---|
| 守护起来后 rank 不从 0 起 / 日志无 `master activated` | 本机有残留 store/守护在跑：`pgrep -f memfabric_daemon`、`ss -lntp \| grep 8587`，kill 后重启 |
